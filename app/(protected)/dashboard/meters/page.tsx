...
import { Ban, Edit, ExternalLink } from "lucide-react";
  ...
  const handleUpdateMeter = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        setMeters(
          meters.map((m) => (m.id === editingMeter.id ? data.meter : m))
        );
    ...
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-neutral-medium mt-2">
            Gerir e consultar histórico de contadores
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {filterStatus === "all" &&
              "Lista completa de todos os contadores registados"}
            {filterStatus === "active" && "Contadores actualmente em serviço"}
            {filterStatus === "replaced" &&
              "Contadores substituídos ou removidos de serviço"}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    {filterStatus !== "active" && (
                      <TableHead>Data de Substituição</TableHead>
                    )}
                ...
                <TableBody>
                  {filteredMeters.map((meter) => (
                    <TableRow key={meter.id}>
                      ...
                      <TableCell>
                        {meter.contract?.client? (
                          <button
                            onClick={() =>
                              router.push(
                                `/dashboard/clients/${
                                  meter.contract!.client!.id
                                }`
                              )
                            }
                            className="text-primary hover:underline flex items-center gap-1 cursor-pointer"
                          >
                            {meter.contract.client.name}
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Actualizar informações do contador {editingMeter?.meterNumber}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input
                id="serial-number"
                value={editingMeter?.serialNumber || ""}
                disabled
              />
            </div>
            <div>
